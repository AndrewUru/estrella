...
import { Music, Upload, ImageIcon, Loader2, CheckCircle2, AlertTriangle } from "lucide-react";
...
export default function NewPracticeForm({ onCreated }: { onCreated?: () => void }) {
  ...
  const canSubmit = useMemo(() => state.title.trim().length > 2 && !!state.kind && !submitting, [state.title, state.kind, submitting]);

  function onChange<K extends keyof FormState>(key: K, value: FormState[K]) {
    setState((s) => ({ ...s, [key]: value }));
  }

  async function uploadToBucket(bucket: "media-audio" | "media-covers", file: File): Promise<string> {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const duration_minutes = state.duration ? parseInt(state.duration, 10) : null;
      const recorded_at = state.recordedAt ? new Date(state.recordedAt).toISOString() : null;
      ...
      console.error(err);
      const msg = err instanceof Error ? err.message : "Error al crear la práctica.";
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <header className="flex items-center gap-3">
        <div className="p-2 rounded-2xl bg-indigo-100 text-indigo-600"><Music className="w-5 h-5" /></div>
        <div>
          <h2 className="text-xl font-semibold">Nueva meditación / canalización</h2>
          <p className="text-sm text-muted-foreground">Sube el audio, añade detalles y publica cuando estés lista.</p>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          <label className="text-sm font-medium flex items-center gap-2"><Music className="w-4 h-4"/> Audio (mp3/wav/m4a)</label>
          <div className="border border-dashed border-border rounded-2xl p-4 flex items-center justify-between">
            ...
            />
            <span className="text-xs text-muted-foreground flex items-center gap-1"><Upload className="w-4 h-4"/>Subir</span>
          </div>
          {state.audioFile && (
            <p className="text-xs text-muted-foreground">Seleccionado: {state.audioFile.name}</p>
          )}
        </div>

        <div className="space-y-2">
          <label className="text-sm font-medium flex items-center gap-2"><ImageIcon className="w-4 h-4"/> Portada (jpg/png)</label>
          <div className="border border-dashed border-border rounded-2xl p-4 flex items-center justify-between">
            <input
              type="file"
              accept="image/*"
          ...
          {state.coverFile && (
            <p className="text-xs text-muted-foreground">Seleccionado: {state.coverFile.name}</p>
          )}
        </div>
      </div>

      <div className="flex items-center gap-3">
        <Button type="submit" disabled={!canSubmit} className={cn("inline-flex items-center gap-2 rounded-2xl", canSubmit ? "bg-indigo-600 text-white" : "bg-neutral-200 text-neutral-500")}
        >
      ...
      <p className="text-xs text-muted-foreground">
        Consejo: deja la visibilidad en <b>private</b> mientras revisas y luego cámbiala a <b>public</b>.
      </p>
    </form>
  );
}
